window.onload = function() {
  ...
  function selectEmployee(block) {
    ...
    employees[index].gender ? getNode("employeeGender").innerText = employees[index].gender : getNode("employeeGender").innerText = "Не вказано";
  }

  function removeEmployee() {
    var bool = confirm("Ця дія приведе до видалення даних про працівника. Продовжити?");
    if (bool) {
  ...
  function editEmployee() {
    ...
    els.gender.value = employees[index].gender;
    els.photo.value = "";
    getNode("photo").src = employees[index].photo;
    getNode("overflow").vis();
    getNode("title").innerText = "Змінення даних працівника";
    getNode("submit").innerText = "Змінити";
    edit.change = true;
    edit.index = index;
  }
  ...
  form.addEventListener("submit",function(event){
    ...
    .then((json) => {
      ...
      if (!edit.change) {
        ...
        li.innerHTML = '<span>'+json.name+'</span><div class="flexed"><div class="option edit" title="Змінити">✏</div><div class="option remove" title="Видалити">×</div></div>';
